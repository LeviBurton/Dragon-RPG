...
namespace RPG.Characters
{
    ...
    public class CharacterSystem : MonoBehaviour
    {
        ...
        void Awake()
        {
          ...
          //  characterController.radius = characterConfig.GetColliderRadius();
          //  characterController.height = characterConfig.GetColliderHeight();
            ...
            aiAgent.updatePosition = true;  // TODO: figure out why we can't match the aiAgents desired velocity!
        ...
        public void Move(Vector3 move)
        {
            ...
            // TODO: should speed be factored in here, or in the caller?
            move = move * CurrentForwardSpeed * Time.deltaTime;
        ...
        void UpdateAnimatorMovement()
        {
            ...
            // Side velocity
            float velocityX = transform.InverseTransformDirection(currentVelocity).x / maxSideSpeed;

            //// Forward velocity
            float velocityZ = transform.InverseTransformDirection(currentVelocity).z / maxForwardSpeed;
        ...
        void UnRegisterSelectableEventHandlers()
        {
            selectable = GetComponent<Selectable>();
        ...
        bool IsAtTarget()
        {
            bool isAtTarget = Vector3.Distance(transform.position, target.transform.position) <= aiAgent.stoppingDistance;
         //   Debug.Log("IsAtTarget: " + isAtTarget);
            return isAtTarget;
        }
        ...
        bool MoveToTarget()
        {
            ...
            //Debug.LogFormat("ai_updatePosition: {0}", aiAgent.updatePosition);
            //Debug.LogFormat("ai_updateRotation: {0}", aiAgent.updateRotation);
            //Debug.LogFormat("ai_steeringTarget: {0}", aiAgent.steeringTarget);

            //Debug.LogFormat("ai_destination: {0}", aiAgent.destination);
            //Debug.LogFormat("ai_velocity: {0}", aiAgent.velocity.ToString());
            //Debug.LogFormat("ai_desiredVelocity: {0}", aiAgent.desiredVelocity.ToString());
        ...
        bool FindThreatenedHeroes()
        {
            ...
            if (lowestHealthHero != null)
            {
                ...
                foreach (var enemy in enemies)
                {
                    ...
                    if (enemy.GetComponent<CharacterSystem>().GetTarget() != lowestHealthHero.gameObject)
                        continue;
